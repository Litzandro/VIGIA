(function(){
  ...
  function renderShifts(){
    ...
    rows.forEach(x=>{
      ...
      el.innerHTML=`<div class="queue-number"><i class="bi bi-clock-history"></i></div><div class="queue-copy"><b>${escapeHtml(x.guardia_original_nombre||`Guardia #${x.guardia_original_id}`)}</b><span>${new Date(x.inicio_programado).toLocaleString('es-HN',fmt12h)} — ${new Date(x.fin_programado).toLocaleString('es-HN',fmt12h)}</span>${x.guardia_relevo_nombre?`<span>Relevo: ${escapeHtml(x.guardia_relevo_nombre)}</span>`:''}<span>${escapeHtml(x.observaciones||'Sin observaciones')}</span></div><div class="queue-actions" style="grid-column:1 / -1;justify-content:flex-start;margin-top:.6rem;"><span class="badge ${x.estado==='activo'?'ok':x.estado==='programado'?'pending':'neutral'}">${escapeHtml(x.estado)}</span>${x.estado==='programado'?'<button class="btn btn-solid" data-a="iniciar">Iniciar</button>':''}${canManage&&['programado','activo'].includes(x.estado)?'<button class="btn btn-ghost" data-a="relevar">Relevar</button>':''}${['activo','relevado'].includes(x.estado)?'<button class="btn btn-ghost" data-a="finalizar">Finalizar</button>':''}</div>`;
