...
(function(){
  ...
  function setStatus(iconClass,text){const i=document.createElement('i');i.className=`bi ${iconClass}`;status.replaceChildren(i,document.createTextNode(' '+text))}
  async function save(next){const p=VigiaAccessibility.set(next);refresh();setStatus('bi-cloud-check-fill','Preferencias guardadas');try{await VigiaAPI.request('/preferencias-usuario/me',{method:'PUT',body:JSON.stringify(mapToApi(p)),offline:false})}catch(e){setStatus('bi-device-ssd','Guardado en este dispositivo')}clearTimeout(status._t);status._t=setTimeout(()=>setStatus('bi-check-circle','Preferencias guardadas automáticamente'),1800)}
