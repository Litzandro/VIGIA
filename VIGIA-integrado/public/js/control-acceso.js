  ...
  async function loadQueue(){
    ...
    try{
      ...
      rows.forEach((x,i)=>{
        ...
        el.innerHTML=`<div class="queue-number">${i+1}</div><div class="queue-copy"><b>${escapeHtml(x.nombre_persona)}</b><span>${escapeHtml(x.vivienda_destino||'Sin destino')} · ${escapeHtml(x.placa_vehiculo||'Sin placa')} · ${time}</span>${flag}</div><div class="queue-actions">${badge(x.estado)}${x.estado==='esperando'?`<button class="btn btn-info" data-a="iniciar">Iniciar validación</button>`:''}${x.estado==='en_validacion'?`<button class="btn btn-solid" data-a="autorizar">Autorizar</button><button class="btn btn-danger" data-a="rechazar">Rechazar</button>`:''}${x.estado==='autorizada'?`<button class="btn btn-solid" data-a="completar">Completar ingreso</button>`:''}</div>`;
